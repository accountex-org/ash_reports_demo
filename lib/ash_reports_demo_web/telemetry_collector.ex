defmodule AshReportsDemoWeb.TelemetryCollector do
  ...
  def handle_call(:get_performance_stats, _from, state) do
    case :ets.lookup(@table_name, :metrics) do
      ...
      [] ->
        {:reply,
         %{requests_per_second: 0, error_rate: 0, cache_hit_rate: 0, performance_score: 0}, state}
    end
  end
  ...
  def handle_endpoint_stop(_event_name, measurements, _metadata, _config) do
    ...
    update_metrics(fn metrics ->
      # Keep last 100
      new_request_times = [duration | Enum.take(metrics.request_times, 99)]
      # Convert to ms
      avg_time = Enum.sum(new_request_times) / length(new_request_times) / 1_000_000
  ...
  def handle_lv_mount_stop(_event_name, measurements, _metadata, _config) do
    ...
    update_metrics(fn metrics ->
      ...
      avg_time =
        if length(new_mount_times) > 0 do
  ...
  def handle_chart_data_query_stop(_event_name, measurements, metadata, _config) do
    ...
    try do
      AshReportsDemo.Resources.TelemetryEvent.create!(
        :chart_data_query,
        to_string(chart_name),
        duration,
        %{
    ...
    update_metrics(fn metrics ->
      ...
      avg_time =
        if length(new_query_times) > 0 do
  ...
  def handle_chart_generate_stop(_event_name, measurements, metadata, _config) do
    ...
    update_metrics(fn metrics ->
      ...
      avg_time =
        if length(new_generation_times) > 0 do
